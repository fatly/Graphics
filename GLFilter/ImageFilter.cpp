...
namespace e
{
	...
	void ImageFilter::SetMatte(unsigned char * buffer, int width, int height, int bitCount)
	{
		if (opengl != 0)
		{
			opengl->SetMatte(buffer, width, height, bitCount);
		}
	}

	void ImageFilter::SetParam(int radius, float simga, bool gray)
	{
		if (opengl != 0)
		{
			opengl->SetParam(radius, simga, gray);
		}
	}
}
